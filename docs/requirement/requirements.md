...
既存の `umya-spreadsheet` 等の代替となる、軽量かつ高速な `.xlsx` (OOXML) パーサーライブラリを開発する。特に日本の業務システムで頻出する「方眼紙Excel（極端に多い列・行）」および「複雑な結合セル」を、メモリを圧迫せずに高速処理・分析し、フロントエンドや他システムで扱いやすいJSON形式で出力することを目的とする。
