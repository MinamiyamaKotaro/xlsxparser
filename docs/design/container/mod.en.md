...
- Provides `has_entry`, an existence-only query backed by a `HashSet<String>` snapshotted once at `open_reader` time — for a caller that only needs to know whether a target exists (never its bytes), this avoids `get_entry`'s local-file-header read and `BoundedReader` construction (Issue #65 PR review; `pipeline.rs`'s image-anchor resolution is the first such caller)
