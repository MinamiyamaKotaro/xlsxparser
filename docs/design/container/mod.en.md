...
3. ~~`max_entry_size` / `max_total_size` configuration interface~~ → **Resolved for now**: implemented as `pub(crate)` builder methods (`with_max_entry_size` / `with_max_total_size`), callable only from within the crate. Not yet exposed on `lib.rs`'s public API — `pipeline.rs` is expected to call them once the public configurability story (security review Finding 2) is designed alongside `lib.rs` (Issue #15).
