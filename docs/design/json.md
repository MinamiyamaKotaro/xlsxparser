...
4. **スタイル情報のJSON出力**: さらに解決が進んだ——`JsonCell.style.font`(Issue #38)、`JsonCell.style.wrapText`(Issue #37)、`JsonCell.style.numberFormat`(Issue #41)、`JsonCell.style.alignment`(Issue #42)、`JsonCell.style.fillFgColor`/`fillBgColor`(Issue #75)をいずれも上記の通り実装した。[model/style.md オープンクエスチョン1](model/style.md) で追跡されていたサブIssueに加え、派生の塗りつぶし色Issueも解決済み。`fillFgColor`/`fillBgColor`は生の指定のまま(`rgb`/`theme`/`indexed`のタグ付き、最終的な表示色ではない)保持する——実RGB値への解決はIssue #76であり、本ファイルのスコープ外。
