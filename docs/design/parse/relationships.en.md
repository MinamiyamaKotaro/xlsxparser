...
1. **Scope of `_rels` parts this file is meant to parse**: currently designed as a generic `_rels` parser not limited to any one part, but this Issue's scope only requires `xl/_rels/workbook.xml.rels` (routing to sheets, shared strings, and styles). Whether relationship types outside the requirements' scope — media embedding (images, etc.), rels for `[Content_Types].xml` — need support is undecided.
...
4. **Namespace handling**: same topic as [parse/mod.md Open Question 4](mod.en.md). The `_rels` XML itself has a fixed namespace (`http://schemas.openxmlformats.org/package/2006/relationships`), but none of its element/attribute names (`Relationship`, `Id`, `Type`, `Target`, `TargetMode`) carry a prefix, so this file is expected to be affected less than other modules.
