...
- **含まない責務**: `ResolvedStyle` をセルへ適用する処理そのもの（[`resolve/style.rs`](../resolve/style.md)）、`ResolvedStyle` / `StyleSheet` / `StyleId` / `Font` / `Alignment` / `ColorRef` の型定義そのもの（[`model/style.rs`](../model/style.md)）、`is_date_time`/`font`/`wrap_text`/`number_format`/`horizontal_alignment`/`fill_fg_color`/`fill_bg_color` 以外の視覚的なスタイル要素の抽出(罫線・垂直方向配置・その他の配置属性。[model/style.md オープンクエスチョン1](../model/style.md) で未解決のまま)、`ColorRef::Theme`/`ColorRef::Indexed` を実際の表示RGB値へ解決すること(Issue #76、diff指向の本パーサには不要な表示用途の関心事)、`applyFont`/`applyAlignment`/`applyNumberFormat`/`cellStyleXfs` に基づく名前付きスタイル継承の解決(下記オープンクエスチョン6参照——`fontId`/`wrapText`/`numFmtId`/`horizontal`/`fillId` を無条件に直接使用する)
