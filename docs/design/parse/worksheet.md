...
- `<sheetData>` の各`<row r="N" ht="..">`を処理する際、`ht`属性があればその場で`push_row_height`を呼び、`Vec<RowHeightRange>`へ圧縮しながら収集する(姉妹プロジェクトexceldiffのIssue #51)——`<col>`と違い`<row>`はOOXMLスキーマ上そもそも範囲を表現できない(1行につき必ず1要素)ため、`<cols>`のように圧縮済みのものを読むだけでは済まず、**圧縮そのものを本ファイルが担う**点が列幅と決定的に異なる(直前のレンジの`max`が現在の行の直前行で、かつ高さが同じなら伸長し、そうでなければ新しいレンジを開始する。詳細・実測は[resolve/row_height.md](../resolve/row_height.md)参照)。`<sheetFormatPr defaultRowHeight="..">`があれば併せて収集する。圧縮済みの`Vec<RowHeightRange>`の検証・登録自体は[`resolve/row_height.rs`](../resolve/row_height.md)が担う
...
- **含まない責務**: 共有文字列インデックス・スタイルIDの実際の解決（[`resolve/shared_strings.rs`](../resolve/shared_strings.md) / [`resolve/style.rs`](../resolve/style.md)）、結合範囲・ハイパーリンク範囲・列幅範囲・行高範囲の妥当性検証・`Sheet` への登録そのもの（[`resolve/merge.rs`](../resolve/merge.md) / [`resolve/hyperlink.rs`](../resolve/hyperlink.md) / [`resolve/column_width.rs`](../resolve/column_width.md) / [`resolve/row_height.rs`](../resolve/row_height.md)。本ファイルは(行高については圧縮済みの)生のリストを収集するのみで `insert_merge`/`finalize_hyperlinks`/`set_col_widths`/`set_row_heights` は呼ばない）、ハイパーリンクの `r:id` を `_rels` に対して解決すること(`pipeline.rs`。ZIP I/Oが必要)、数式（`<f>` 要素）の解析・保持（オープンクエスチョン2参照）
