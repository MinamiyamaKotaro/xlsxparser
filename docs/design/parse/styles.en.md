...
- **Not responsible for**: applying a `ResolvedStyle` to a cell ([`resolve/style.rs`](../resolve/style.en.md)), defining the `ResolvedStyle` / `StyleSheet` / `StyleId` / `Font` types themselves ([`model/style.rs`](../model/style.en.md)), extracting visual style elements beyond `is_date_time`/`font` (fill, border, wrap, alignment — see [model/style.md Open Question 1](../model/style.en.md), still open for those), resolving `applyFont`/`cellStyleXfs`-based named-style inheritance (see Open Question 6 below — `fontId` is used directly and unconditionally)
...
5. **Concrete style elements such as font/fill/border**: partially resolved — `font: Font { size_pt, bold }` (Issue #38) is implemented as described above. Fill, border, wrap, and alignment remain open, tracked as [Issue #36](https://github.com/MinamiyamaKotaro/xlsxparser/issues/36)'s remaining sub-issues (#37, #41, #42).
6. **Support for `applyNumberFormat`/`applyFont` and `cellStyleXfs` (named cell-style inheritance)**: resolved as a deliberate simplification, extended from numFmt to font when Issue #38 landed — both `numFmtId` and `fontId` are treated as authoritative regardless of the `<xf>`'s `applyNumberFormat`/`applyFont` value, with no consideration of the inheritance chain `cellStyleXfs`/`xfId` would otherwise establish. In practice, a real `<xf>` in `<cellXfs>` virtually always carries a valid `fontId`/`numFmtId` pointing at the format it actually uses, regardless of what `applyFont`/`applyNumberFormat` says (a UI hint about whether the *user* explicitly customized that property, not whether the property applies when reading the file back) — full `cellStyleXfs` inheritance is a materially larger feature (resolving named cell styles like "Normal"/"Heading 1") that no downstream use case has asked for yet. Revisit if a concrete case surfaces where this simplification produces a wrong `font`/`is_date_time`.
