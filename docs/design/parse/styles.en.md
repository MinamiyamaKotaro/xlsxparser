...
- **Not responsible for**: applying a `ResolvedStyle` to a cell ([`resolve/style.rs`](../resolve/style.en.md)), defining the `ResolvedStyle` / `StyleSheet` / `StyleId` / `Font` types themselves ([`model/style.rs`](../model/style.en.md)), extracting visual style elements beyond `is_date_time`/`font`/`wrap_text` (fill, border, `number_format`, other `alignment` attributes — see [model/style.md Open Question 1](../model/style.en.md), still open for those), resolving `applyFont`/`applyAlignment`/`cellStyleXfs`-based named-style inheritance (see Open Question 6 below — `fontId`/`wrapText` are used directly and unconditionally)
