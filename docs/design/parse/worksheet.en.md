...
- While processing each `<row r="N" ht="..">` inside `<sheetData>`, calls `push_row_height` right there if `ht` is present, compressing into `Vec<RowHeightRange>` as it goes (sister project exceldiff's Issue #51) — unlike `<col>`, `<row>` can never itself express a range in the OOXML schema (always one element per row), so reading pre-compressed ranges the way `<cols>` allows isn't an option: **the compression itself is this file's own responsibility**, the decisive difference from column width (extends the last range's `max` when the current row immediately follows it at the same height, otherwise starts a new one — see [resolve/row_height.en.md](../resolve/row_height.en.md) for the full rationale and measurements). Also collects `<sheetFormatPr defaultRowHeight="..">` if present. Validating and registering the already-compressed `Vec<RowHeightRange>` itself is [`resolve/row_height.rs`](../resolve/row_height.en.md)'s job
...
- **Not responsible for**: actually resolving shared-string indices or style IDs ([`resolve/shared_strings.rs`](../resolve/shared_strings.en.md) / [`resolve/style.rs`](../resolve/style.en.md)), validating merge ranges, hyperlink ranges, column-width ranges, or row-height ranges, or registering any of them into `Sheet` ([`resolve/merge.rs`](../resolve/merge.en.md) / [`resolve/hyperlink.rs`](../resolve/hyperlink.en.md) / [`resolve/column_width.rs`](../resolve/column_width.en.md) / [`resolve/row_height.rs`](../resolve/row_height.en.md) — this file only collects the raw (already-compressed, for row height) lists; it never calls `insert_merge`/`finalize_hyperlinks`/`set_col_widths`/`set_row_heights`), resolving a hyperlink's `r:id` against `_rels` (`pipeline.rs`, needs ZIP I/O), parsing or retaining formulas (`<f>` elements — see Open Question 2)
...
- The same applies if a `<row>`'s `ht`, or `<sheetFormatPr>`'s `defaultRowHeight`, cannot be parsed as `f64`: `Error::InvalidPackage`. Range-count validation (`MAX_ROW_HEIGHT_RANGES`) is left to [`resolve/row_height.rs`](../resolve/row_height.en.md), but the compression itself (`push_row_height`) is this file's own responsibility, unlike column width (see "Responsibilities / Scope" above)
- `width`/`defaultColWidth`/`ht`/`defaultRowHeight` all go through the same shared `parse_f64_attr`, which checks not just that the value parses as `f64` but that it's **finite and non-negative** — `NaN`, `±Infinity`, and negative numbers are all rejected as `Error::InvalidPackage`. Without this, `json.rs` would serialize `width`/`heightPt` straight through, and `serde_json` errors on a non-finite `f64` (JSON has no representation for `NaN`/`Infinity`), so a malformed file would surface as a confusing JSON-export failure rather than being rejected up front where the bad value actually came from (a Copilot review on sister project exceldiff's PR #53 caught this; the fix protects column width the same way, since it shares the helper)
...
- Verify that `<sheetFormatPr defaultRowHeight="..">` is collected, and that its absence leaves `default_row_height: None`
- Verify the compression algorithm itself (sister project exceldiff's Issue #51, `push_row_height`): consecutive same-height rows compress into one range; a gap in row numbers (a row with no explicit `<row ht>` in between) starts a new range even at the same height; a row with no `ht` attribute contributes nothing
- Verify that a malformed `<col>`/`<sheetFormatPr>` numeric attribute returns `Error::InvalidPackage`
- Verify that `NaN`, `inf`, `-inf`, and a negative number (`-1.0`) are all rejected as `Error::InvalidPackage` for each of `ht`, `defaultRowHeight`, and `width` (a Copilot review on sister project exceldiff's PR #53; added as a regression test since column width shares the same `parse_f64_attr`)
