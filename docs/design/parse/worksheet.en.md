...
- `<row>` and `<c>`'s `r` attribute are both optional; when omitted, the position is inferred from the previous row/cell (Issue #79 — resolves former Open Question 4). Returns `Error::InvalidCellRef` for `<row r="0">`, a `<row>` `r` beyond `CellRef::MAX_ROW`, or an inferred column beyond `CellRef::MAX_COL` — the same convention `CellRef::from_a1` already applies. Also `Error::InvalidCellRef` if a `<c>` omits `r` before any `<row>` has been seen at all
...
- Verify that each returns `Error::InvalidCellRef`: a `<c>` omitting `r` before any `<row>` has been seen, a `<row r="0">`, a `<row>` `r` beyond `CellRef::MAX_ROW`, and an inferred column beyond `CellRef::MAX_COL` (a regression test for the same attack surface security review Finding 2 covers) (Issue #79)
