...
- Exceeding `limits.max_entry_size` / `limits.max_total_size` surfaces as `Error::ZipBombDetected`, returned by `pipeline::run` (via [container/mod.md](container/mod.en.md)'s `BoundedReader`) — exactly the same error path as when the default cap is used. Neither the `_with_limits` functions nor the default-cap functions add any new error handling on this front
