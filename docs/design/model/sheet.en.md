...
**Correction (sister project exceldiff's Issue #50): "never discards observable data" was inaccurate for borders.** The claim above is correct about *reachability* — a non-origin coordinate was already unreachable through `get`/`iter_cells` — but it missed that a non-origin cell's own border style isn't necessarily dead data: it can still be the information that should contribute to the merged region's visible outline. The sister project exceldiff demonstrated this concretely against a real `.xlsx` (a "skill sheet" template): Excel had recorded a merge's right border as `<right style="...">` on the rightmost *non-origin* cell (e.g. `Q2` of an `H2:Q2` merge) rather than on the origin. exceldiff surfaced this through its own grid-rendering module (`grid.rs`, which this crate has no equivalent of), but the same information loss affects any consumer of this crate that reads `Cell::style` after parsing — this crate's own JSON export included.

The fix: right before a non-origin cell is pushed onto `to_drop`, `fold_perimeter_border_into_origin` checks whether it sits on the region's outer edge (top row, left column, bottom row, right column) and, for each edge it touches, ORs that side's border into the origin's own `Borders` — a cell that touches no edge (interior to the merge) never contributes, since its border couldn't have been part of the visible outline anyway. Because the origin's style is a `Option<Arc<ResolvedStyle>>` potentially shared with other cells, the update goes through `Arc::make_mut` (clones only if the reference count isn't 1) rather than mutating through the existing `Arc` directly — otherwise the border would spuriously leak onto every other cell sharing that same style (regression test `finalize_merges_border_fold_never_mutates_a_style_shared_with_unrelated_cells` — the *origin* itself starts out sharing an Arc with an unrelated cell, and the assertion compares against a value snapshotted before `finalize_merges` runs, since comparing against the post-fold `&*shared_before` reference wouldn't catch an in-place mutation — both sides would alias the same, now-mutated, allocation). The fold-in is O(1) per covered cell inside the existing `SweepEvent::Query` match arm, so the sweep's own O((C + M) log (C + M)) bound is unchanged.
...
**Feature: row height (sister project exceldiff's Issue #51).** `Sheet` mirrors column width on the row axis: `row_heights: Vec<RowHeightRange>` (sorted by `min`, mutually non-overlapping) and `default_row_height: Option<f64>`, populated once by `resolve::row_height::resolve` via `Sheet::set_row_heights`. The decisive difference from column width is the *shape of the source data*: `<col min=".." max="..">` is itself a range in the OOXML schema, but `<row r="N" ht="..">` is always one element per row — the schema has no concept of a row range at all. So unlike `col_width_ranges`, which just reads pre-compressed ranges straight off `<cols>`, the compression for `row_height_ranges` happens in `parse/worksheet.rs` itself, while streaming `<sheetData>` row by row (`push_row_height`: extends the last `RowHeightRange` if the current row immediately follows it at the same height, otherwise starts a new one) — `resolve::row_height::resolve` receives an already-compressed `Vec<RowHeightRange>`, a structural difference from `resolve::column_width::resolve` (see [parse/worksheet.en.md](../parse/worksheet.en.md) and [resolve/row_height.en.md](../resolve/row_height.en.md) for the details).

`row_height(row) -> Option<f64>` is the exact same binary-search shape as `column_width` (`partition_point` over `row_heights`), returning `None` when nothing covers `row` and no `defaultRowHeight` was set. exceldiff's `grid.rs` (which converts points to pixels for on-screen rendering via `excel_height_pt_to_px`) has no counterpart in this crate, so this crate only exposes the raw point value — `row_height_ranges()`/`default_row_height()` are exposed in exactly the same shape as column width's `col_width_ranges()`/`default_col_width()`, and `json.rs` serializes them as sheet-level `rows`/`defaultRowHeight` fields (never duplicated per cell, the same sparse-output reasoning).
...
- **Verifying `finalize_merges` folds a merged region's outer-edge borders into its origin** (sister project exceldiff's Issue #50): both a single-row merge (only the right edge is exercised) and a genuinely 2D merge (all four edges, each carried by a different cell, plus an interior cell that must *not* contribute)
- **Verifying that fold-in never mutates a `Arc<ResolvedStyle>` shared with an unrelated cell** (same Issue #50; regression coverage that the `Arc::make_mut` COW split actually works — the origin itself starts out sharing an Arc with an unrelated cell, and the comparison is against a value snapshotted before the fold runs)
