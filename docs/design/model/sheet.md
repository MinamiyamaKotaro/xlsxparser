...
**訂正(姉妹プロジェクトexceldiffのIssue #50): 「観測可能なデータを一切失わない」は罫線について不正確だった。** 上記の主張は、非起点セルが`get`/`iter_cells`からもともと到達不能だった、という**到達可能性**の意味では正しいが、非起点セル自身の罫線情報がそもそも「死んでいたデータ」ではなく、結合範囲の見た目の外枠に寄与すべき情報だったケースを見落としていた。姉妹プロジェクトexceldiffが実データ(`.xlsx`の「スキルシート」テンプレート)で確認したところ、`H2:Q2`のような結合セルの右罫線をExcelが**右端の非起点セル(Q2)自身**の`<right style="...">`として保持している例が実在した。exceldiffにはこの罫線を画面描画する`grid.rs`があるためそちらで顕在化したが、本クレート自体にも(パース後に`Cell::style`を読む全ての利用者に影響する形で)同じ情報欠落がある。

対処として、`finalize_merges`が非起点セルを`to_drop`へ積む直前に`fold_perimeter_border_into_origin`を呼び、そのセルが結合範囲の外周(上端行/左端列/下端行/右端列)に位置する場合は該当する辺の罫線を起点セルの`Borders`へOR演算で畳み込んでから削除するようにした——結合範囲の内部(どの辺にも接しない)セルの罫線は外枠に寄与しないため対象外。起点セルのスタイルは`Option<Arc<ResolvedStyle>>`で他のセルと共有されうるため、既存のArcを直接書き換えず`Arc::make_mut`(参照カウントが1でなければ内部でクローンしてから可変参照を返す)で分離してから更新する——さもないと同じスタイルを共有する無関係なセルにまで罫線が波及してしまう(回帰テスト`finalize_merges_border_fold_never_mutates_a_style_shared_with_unrelated_cells`——起点セル自身を無関係セルとArcを共有させた上で、判定は畳み込み前にスナップショットした値との比較で行う。畳み込み後の`&*shared_before`参照との比較では、in-place変異が起きていても両辺が同じ(変異済みの)実体を指すため検出できない)。この畳み込みは`SweepEvent::Query`の一致判定内で行毎セルにつきO(1)であり、スイープライン全体の計算量O((C + M) log (C + M))は変わらない。
...
**機能: 行の高さ(姉妹プロジェクトexceldiffのIssue #51)。** `Sheet` は列幅と対称に `row_heights: Vec<RowHeightRange>`(`min` でソート済み・相互に非重複)と `default_row_height: Option<f64>` を保持し、`resolve::row_height::resolve` が `Sheet::set_row_heights` を通じて一度だけ登録する。列幅との決定的な違いは**元データの形状**にある: `<col min=".." max="..">` はOOXMLのスキーマ自体が範囲として書けるのに対し、`<row r="N" ht="..">` は必ず1行につき1要素で、範囲という概念がスキーマに存在しない。そのため`col_width_ranges`が`<cols>`から圧縮済みの範囲をそのまま読むだけなのと異なり、`row_height_ranges`の圧縮そのものは`parse/worksheet.rs`が`<sheetData>`を1行ずつストリーム処理しながら行う(直前の`RowHeightRange`の`max`が現在の行の直前で、かつ高さが同じなら伸長し、そうでなければ新しいレンジを開始する`push_row_height`)——`resolve::row_height::resolve`が受け取る時点で既に圧縮済みの`Vec<RowHeightRange>`である点が`resolve::column_width::resolve`との構造上の違い(詳細は[parse/worksheet.md](../parse/worksheet.md)・[resolve/row_height.md](../resolve/row_height.md)参照)。

`row_height(row) -> Option<f64>` は`column_width`と全く同じ二分探索の形(`row_heights`を`partition_point`)で、`row`を覆う範囲も`defaultRowHeight`も無ければ`None`を返す。exceldiffの`grid.rs`(画面描画用にptをpxへ変換する`excel_height_pt_to_px`を持つ)は本クレートには存在しないため、本クレートが公開するのは生のpt値までで止まる——`row_height_ranges()`/`default_row_height()`は列幅の`col_width_ranges()`/`default_col_width()`と全く同じ形で公開しており、`json.rs`がシート単位の`rows`/`defaultRowHeight`配列としてシリアライズする(セルごとに複製しない、という同じ疎な出力設計の理由)。
...
- **`finalize_merges` が結合範囲の外周罫線を起点セルへ畳み込むことの確認**（姉妹プロジェクトexceldiffのIssue #50。1行結合(右端のみ)・2次元結合(4辺すべてを別々のセルが担うケース、内部セルは対象外であることを含む)の両方を検証)
- **その畳み込みが、他の無関係なセルと共有された`Arc<ResolvedStyle>`を書き換えないことの確認**（同Issue #50。`Arc::make_mut`によるCOW分離が実際に機能していることの回帰テスト——起点セル自身を無関係セルとArcを共有させ、畳み込み前にスナップショットした値と比較する)
