...
1. **Handling of hidden sheets**: A `<sheet>` element in `workbook.xml` can have `state="hidden"` / `"veryHidden"`. Whether `Workbook.sheets` should include all sheets regardless of visibility, include only visible sheets, or have `Sheet` carry a visibility field and let the caller filter, is undecided. Needs confirmation since the requirements spec doesn't specify this.
